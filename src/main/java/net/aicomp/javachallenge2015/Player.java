...
public class Player {
	...
	private boolean runPlayer() {
		...
		try {
			...
			System.err.println("ERROR: Unable to execute the command: " + execCommand + ".");
			aiProcess = null;
			return false;
		}

		reader = new BufferedReader(new InputStreamReader(aiProcess.getInputStream()));
	...
	// AIに情報を送るって行動コマンドを得る
	public String getAction(int playerID, int turn, int[][] board, ArrayList<Integer> lifeList,
	...
	private boolean commandExec(String command) {
		...
		if (command != null) {
			try {
				...
				System.err.println("ERROR: Unable to execute the command: " + command + ".");
				return false;
			}
		}
		return true;
